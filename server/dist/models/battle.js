...
const battle_rules_1 = require("./battle-rules");
const types_1 = require("../types");
const config_1 = require("../config");
const BALL_INFO = [
    { type: types_1.PokeBallType.PokeBall, name: '精灵球', catchRate: 1.0 },
    { type: types_1.PokeBallType.GreatBall, name: '超级球', catchRate: 1.5 },
    { type: types_1.PokeBallType.UltraBall, name: '高级球', catchRate: 2.0 },
    { type: types_1.PokeBallType.MasterBall, name: '大师球', catchRate: 5.0 },
];
class Battle {
    constructor(player) {
        this.player = player;
    ...
    start(wildPokemonSpecies) {
        this.inBattle = true;
        this.wildPokemon = new pokemon_1.Pokemon(wildPokemonSpecies, 5);
        this.actionLog = [];
        // 找到第一只存活的宝可梦出战
        this.playerPokeIndex = 0;
    ...
    enemyTurn() {
        if (!this.wildPokemon || !this.inBattle)
            return;
        this.actionLog.push('');
    ...
    useMove(attacker, defender, moveIndex, isPlayer) {
        ...
        if (move.power > 0) {
            ...
            const damage = (0, battle_rules_1.calculateDamage)(attacker, defender, move, randomFactor);
            const effectiveness = (0, type_chart_1.getTypeEffectiveness)(move.type, defender.type);
            if (effectiveness === 0) {
                this.actionLog.push(`🚫 对${(0, type_chart_1.getTypeName)(defender.type)}属性无效！`);
                return;
            }
    ...
    handleCatch(ballIndex) {
        ...
        const catchProbability = (0, battle_rules_1.calculateCatchProbability)(enemyPoke.stats.hp, enemyPoke.stats.maxHp, enemyPoke.level, ball.catchRate, ballIndex === 3);
        ...
        if (roll < catchProbability) {
            ...
            this.player.gold += config_1.GOLD_CATCH_POKEMON;
            this.actionLog.push(`💰 获得了 ${config_1.GOLD_CATCH_POKEMON} 金币！`);
    ...
    handleHeal() {
        ...
        const healAmount = Math.max(1, Math.floor(playerPoke.stats.maxHp * config_1.HEAL_POTION_PERCENT / 100));
    ...
    handleEscape() {
        const playerPoke = this.player.team[this.playerPokeIndex];
        const enemyPoke = this.wildPokemon;
        const escapeChance = (0, battle_rules_1.calculateEscapeChance)(playerPoke.stats.speed, enemyPoke.stats.speed);
    ...
    checkBattleEnd() {
        ...
        if (enemyPoke.isFainted()) {
            ...
            this.player.gold += config_1.GOLD_WIN_BATTLE;
            this.actionLog.push(`💰 获得了 ${config_1.GOLD_WIN_BATTLE} 金币！`);
            playerPoke.exp += config_1.EXP_BASE_GAIN;
            if (playerPoke.exp >= playerPoke.maxExp) {
                playerPoke.level++;
                playerPoke.maxExp += config_1.EXP_LEVEL_BASE;
